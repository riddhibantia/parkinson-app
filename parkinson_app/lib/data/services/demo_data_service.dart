  ...
  static List<TypingSession> demoSessions() {
    ...
    for (var i = 0; i < 2; i++) {
      out.add(
        TypingSession(
          ...
          endTime: now
              .subtract(Duration(days: 7, hours: i))
    ...
    for (var i = 0; i < 8; i++) {
      ...
      final start = now.subtract(
        Duration(days: dayOffset, hours: rng.nextInt(6)),
      );
      out.add(
        TypingSession(
          sessionId: 'demo-s-$i',
    ...
    'top_contributors': [
      {
        'feature': 'ht_mean',
        'text':
            'Your key presses held slightly longer than typical this session.',
      },
      {
        'feature': 'pause_frequency',
        'text': 'Your typing included more pauses than typical this session.',
      },
    ],
  };
  ...
  static Map<String, dynamic> demoLayer2Result() => {
    ...
    'drift_result': {
      ...
      'drift_signals': {
        'ht_mean': {
          'drift_detected': true,
          'robust_z_current': 2.1,
          'trend': 'increasing',
          'unusual_range': true,
        },
        'ft_mean': {
          'drift_detected': false,
          'robust_z_current': 0.8,
          'trend': 'stable',
          'unusual_range': false,
        },
        'typing_speed': {
          'drift_detected': false,
          'robust_z_current': -0.5,
          'trend': 'stable',
          'unusual_range': false,
        },
        'pause_frequency': {
          'drift_detected': false,
          'robust_z_current': 1.2,
          'trend': 'stable',
          'unusual_range': false,
        },
      },
